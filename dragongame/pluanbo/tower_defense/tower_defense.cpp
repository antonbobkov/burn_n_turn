...
void DrawStuff(Rectangle rBound, SP<Graphic> pGraph, SP<Soundic> pSnd, MyPreloader& pr, int n)
{
...
TwrGlobalController::TwrGlobalController(SP<ScalingDrawer> pDr_, SP<NumberDrawer> pNum_, SP<NumberDrawer> pBigNum_,
										 SP<FontWriter> pFancyNum_,
										 SP<Soundic> pSndRaw_, const LevelStorage& vLvl_, Rectangle rBound_, TowerDataWrap* pWrp_, FilePath fp)
    :nActive(1), pDr(this, pDr_), pGraph(this, pDr_->pGr), pNum(this, pNum_), pBigNum(this, pBigNum_), pr(pDr_->pGr, pSndRaw_, fp),
	pSndRaw(this, pSndRaw_), pSnd(this, new SoundInterfaceProxy(pSndRaw)),
	nScore(0), vLvl(vLvl_), rBound(rBound_), bAngry(false), nHighScore(0), pFancyNum(this, pFancyNum_), pWrp(pWrp_),
	pMenu(this, 0), vLevelPointers(3),
	sbTutorialOn("tutorial_on.txt", true, true),
	...
	sbCheatsUnlocked("more_stuff.txt", false, true)
{
  ...
  //pr.LoadSeqTS("bonus\\burning.txt", "burning_bonus");	// 0 - time (not used)
  ...
  //pr.LoadSeqTS("bonus\\frequency.txt", "frequency");		// 11 - frequency (not used)
    ...
    pr.LoadSeqTS("castle\\destroy_castle_dust.txt", "destroy_castle", Color(0, 0, 0));
	...
	pr.LoadSeqTS("explosion\\explosion2.txt", "explosion_2", Color(0,0,0,0), nScale*2);
    pr.LoadSeqTS("explosion\\laser_expl.txt", "laser_expl_2", Color(0,0,0,0), nScale*2);
    
    pr.LoadSeqTS("explosion\\explosion_15.txt", "explosion_3", Color(0,0,0,0), nScale*2);
    pr.LoadSeqTS("explosion\\laser_expl_15.txt", "laser_expl_3", Color(0,0,0,0), nScale*2);

	pr.LoadSeqTS("fireball\\fireball.txt", "fireball");
    pr.LoadSeqTS("fireball\\fireball_15.txt", "fireball_15");
    pr.LoadSeqTS("fireball\\fireball.txt", "fireball_2", Color(0,0,0,0), nScale*2);
    pr.LoadSeqTS("fireball\\fireball_15.txt", "fireball_3", Color(0,0,0,0), nScale*2);
    ...
    pr.LoadSeqTS("fireball\\laser_15.txt", "laser_3", Color(0,0,0,0), nScale*2);
	pr.LoadSeqTS("fireball\\fireball_icon.txt", "fireball_icon", Color(255,255,255));
...
#else //save memory on trial, though, *something* needs to be loaded otherwise crashes.
...
void TwrGlobalController::StartUp()
{
	...
	SP<Animation> pStr = new Animation(0, pr("start"), nFramesInSecond/5, Point(rBound.sz.x/2,rBound.sz.y*3/4), true);

	// menu
	SP<MenuController> pMenuHolder = new MenuController(this, rBound, Color(0,0,0), 3); // resume position shouldn't matter
	pMenu = pMenuHolder;
	
	
	// logo 1
	SP<BasicController> pCnt0_1 = new AlmostBasicController( BasicController(this, rBound, Color(0, 0, 0)));
	// logo 2
    SP<BasicController> pCnt0_2 = new AlmostBasicController( BasicController(this, rBound, Color(0, 0, 0)));
    // press start screen
	SP<BasicController> pCnt1 = new StartScreenController(this, rBound, Color(0, 0, 0));
    // game over
	SP<BasicController> pCnt2 = new AlmostBasicController( BasicController(this, rBound, Color(0, 0, 0)));
    // you win!
	SP<BasicController> pCnt3 = new BasicController(this, rBound, Color(0, 0, 0));
	// score
	SP<DragonScoreController> pScore = new DragonScoreController(this, rBound, Color(0, 0, 0), true);
	// intro tutorial screen (non PC version)
	SP<DragonScoreController> pIntro = new DragonScoreController(this, rBound, Color(0, 0, 0), false);

    // cutscenes
	SP<BasicController> pCut1 = new Cutscene(this, rBound, "princess", "knight");
    SP<BasicController> pCut2 = new Cutscene(this, rBound, "knight", "dragon_walk_f", true);
    ...
    SP<Animation> pWin = new Animation(0, pr("win"), 3, Point(rBound.sz.x/2,rBound.sz.y/2 - 20), true);
    SP<StaticImage> pL = new StaticImage(pr["logo"],Point(rBound.sz.x/2,rBound.sz.y/3), true);
	SP<Animation> pBurnL = new Animation(0, pr("burn"), 3, Point(rBound.sz.x/2-45,rBound.sz.y/2 - 64), true);
	SP<Animation> pBurnR = new Animation(0, pr("burn"), 4, Point(rBound.sz.x/2-54,rBound.sz.y/2 - 64), true);
	...
	vHints.push_back("pick up traders by clicking when flying over them\nbring them to the tower to get longer bonuses");
#endif
	vHints.push_back("this game was originally called tower defense");

	std::string sHint = vHintPref.at(rand()%vHintPref.size()) + vHints.at(rand()%vHints.size());
		
	SP<TextDrawEntity> pHintText = new TextDrawEntity(0, Point(rBound.sz.x/2, rBound.sz.y*7/8), true, sHint, pNum); 
	SP<TextDrawEntity> pOptionText = new TextDrawEntity(0, Point(rBound.sz.x/2, rBound.sz.y*7/8), true, "sup", pNum);

	SP<AnimationOnce> pO = new AnimationOnce(0, pr("over"), nFramesInSecond/2, Point(rBound.sz.x/2,Crd(rBound.sz.y/2.5f)), true);
    SP<AnimationOnce> pPlu = new AnimationOnce(0, pr("pluanbo"), nFramesInSecond/10, Point(rBound.sz.x/2,rBound.sz.y/2), true);
    SP<AnimationOnce> pGen = new AnimationOnce(0, pr("gengui"), nFramesInSecond/5, Point(rBound.sz.x/2,rBound.sz.y/2), true);

	SP<SimpleSoundEntity> pOver = new SimpleSoundEntity(pr.GetSndSeq("over"), nFramesInSecond/2, pSnd);
    SP<SimpleSoundEntity> pPluSnd = new SimpleSoundEntity(pr.GetSndSeq("pluanbo"), nFramesInSecond/10, pSnd);
    SP<SimpleSoundEntity> pClkSnd = new SimpleSoundEntity(pr.GetSndSeq("click"), nFramesInSecond/5, pSnd);
    
#ifdef TRIAL_VERSION
	SP<StaticImage> pTrial = new StaticImage(pr["trial"],Point(rBound.sz.x/2-73,rBound.sz.y/3+28), true);
	pCnt1->AddV(pTrial);
#endif

    SP<Animation> pMenuCaret = new Animation(2, pr("arrow"), 3, Point(0, 0), true);
	//menu entity
	SP<MenuDisplay> pMenuDisplay = new MenuDisplay(Point(rBound.sz.x/2-8,rBound.sz.y/2), pNum, pMenuCaret, pMenu, sbCheatsUnlocked.Get());
    ...
    {
        SP<AdvancedController> pAd = new AdvancedController(this, rBound, Color(0, 0, 0), vLvl[i]);
...
#ifdef TRIAL_VERSION
    SP<BuyNowController> pBuy = new BuyNowController(this, rBound, Color(0, 0, 0));

    SP<Animation> pGolem = new Animation(0, pr("golem_f"), nFramesInSecond/10, Point(rBound.sz.x/4, rBound.sz.y*3/4 - 10), true);
    SP<Animation> pSkeleton1 = new Animation(0, pr("skelly"), nFramesInSecond/4, Point(rBound.sz.x*3/4, rBound.sz.y*3/4 - 5), true);
    SP<Animation> pSkeleton2 = new Animation(0, pr("skelly"), nFramesInSecond/4 + 1, Point(rBound.sz.x*3/4 - 10, rBound.sz.y*3/4 - 15), true);
    SP<Animation> pSkeleton3 = new Animation(0, pr("skelly"), nFramesInSecond/4 - 1, Point(rBound.sz.x*3/4 + 10, rBound.sz.y*3/4 - 15), true);
    SP<Animation> pMage = new Animation(0, pr("mage_spell"), nFramesInSecond/2, Point(rBound.sz.x/2, rBound.sz.y*3/4), true);
	SP<Animation> pGhost = new Animation(0, pr("ghost"), nFramesInSecond/6, Point(rBound.sz.x*5/8, rBound.sz.y*3/4-30), true);
	SP<Animation> pWhiteKnight = new Animation(0, pr("ghost_knight"), nFramesInSecond/6, Point(rBound.sz.x*3/8, rBound.sz.y*3/4-30), true);

	//SP<StaticImage> pBuyNow = new StaticImage(pr["buy"],Point(rBound.sz.x/2 - 73,rBound.sz.y/3 + 33), true);
	SP<StaticImage> pBuyNow = new StaticImage(pr["buy"],Point(rBound.sz.x/2,rBound.sz.y/3 + 33), true);
	SP<VisualEntity> pSlimeUpd = new SlimeUpdater(pBuy.GetRawPointer());

	/*
    for(int i = 6; i <= 12; ++i)
	{
		SP<Animation> pSlm = new Animation(0, pr("slime"), nFramesInSecond/10, Point(rBound.sz.x*i/18, rBound.sz.y/2), true);
		pBuy->AddBoth(pSlm);
	}*/
...
:Critter(cr), pBc(this, pBc_),nTimer(nTimer_), nTimer_i(nTimer_),c(c_), t(nFramesInSecond/5)
{
        ...
        if (this->HitDetection(*itr))
        {
            ...
            else
            {
                ...
                SP<KnightOnFire> pKn = new KnightOnFire(
                    Critter(GetRadius(), (*itr)->GetPosition(), fPoint(),
...
struct AdNumberDrawer: public VisualEntity
{
    ...
    /*virtual*/ void Draw(SP<ScalingDrawer> pDr)
    {
...
#ifdef FULL_VERSION
		pAd->pGl->pNum->DrawNumber(pAd->pGl->nScore, Point(pAd->rBound.sz.x - 27*4, 4), 7);
		...
		if(pAd->bBlink)
		{
			...
			pAd->pGl->pNum->DrawColorNumber((pAd->t.nPeriod - pAd->t.nTimer)/nFramesInSecond, Point(pAd->rBound.sz.x - 14*4, 4), c, 4);
			pAd->pGl->pNum->DrawColorWord("time:", Point(pAd->rBound.sz.x - 19*4, 4), c);
		}

        pAd->pGl->pNum->DrawNumber(pAd->nLvl, Point(pAd->rBound.sz.x - 3*4, 4), 2);
        
		pAd->pGl->pNum->DrawWord("score:", Point(pAd->rBound.sz.x - 33*4, 4));
        pAd->pGl->pNum->DrawWord("level:", Point(pAd->rBound.sz.x - 9*4, 4));
		if(pAd->bCh)
		{
			pAd->pGl->pNum->DrawColorWord("invincible", Point(pAd->rBound.sz.x - 44*4, 4), Color(0, 255, 0));
		}
#else
		pAd->pGl->pNum->DrawNumber(pAd->pGl->nScore, Point(pAd->rBound.sz.x - 17*4, 4), 7);
        pAd->pGl->pNum->DrawNumber(pAd->nLvl, Point(pAd->rBound.sz.x - 3*4, 4), 2);
...
struct BonusDrawer: public VisualEntity
{
	...
	BonusDrawer():pAd(this, 0), t(unsigned(nFramesInSecond*.1F)), nAnimationCounter(0){}
    ...
    /*virtual*/ void Draw(SP<ScalingDrawer> pDr)
    {
		...
		for(unsigned nDr = 0; nDr < pAd->vDr.size(); ++nDr)
		{
			...
			for(BonusList::iterator itr = lst.begin(), etr = lst.end(); itr != etr; ++itr)
			{
				
				SP<TimedFireballBonus> pBns = *itr;

				if(pBns->t.nPeriod && (pBns->t.nPeriod - pBns->t.nTimer) < 4*nFramesInSecond)
...
AdvancedController::AdvancedController(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c, const LevelLayout& lvl)
:BasicController(pGl_, rBound, c), bCh(false), nLvl(lvl.nLvl), nSlimeNum(0), bPaused(false), bFirstUpdate(true),bLeftDown(false),bRightDown(false),nLastDir(0),bWasDirectionalInput(0),
bGhostTime(false), bBlink(true), pGr(0), bLeft(false), pSc(0), bTakeOffToggle(false),
pTutorialText(this, 0), mc(pGl->pr("claw"), Point(), pGl.GetRawPointer()), bTimerFlash(false)
{	
	...
	SP<KnightGenerator> pGen = new KnightGenerator(lvl.vFreq.at(0), rBound, this, lvl.blKnightGen);
    SP<PrincessGenerator> pPGen = new PrincessGenerator(lvl.vFreq.at(1), rBound, this);
    SP<TraderGenerator> pTGen = new TraderGenerator(lvl.vFreq.at(2), rBound, this);
    SP<MageGenerator> pMGen = new MageGenerator(lvl.vFreq.at(3), lvl.vFreq.at(4), rBound, this);
	...
	SP<TutorialTextEntity> pTT = new TutorialTextEntity(1, pos, pGl->pNum, pGl->sbTutorialOn.GetConstPointer() );
	pTutorialText = pTT;
...
void AdvancedController::OnMouseDown(Point pPos)
{
	...
	if(!vDr[0]->bFly)
		bHit = (pt.GetDirection(vDr[0]->pCs->GetPosition()).Length() < fTowerClickRadius);
	else
		bHit = (pt.GetDirection(vDr[0]->GetPosition()).Length() < fDragonClickRadius);
...
void AdvancedController::OnMouseUp()
{
	...
	if(vDr[0]->bFly && fTime <= .2 && !bTakeOffToggle && pt.GetDirection(vDr[0]->GetPosition()).Length() > vDr[0]->nRadius )
	{
		fPoint fFb = vDr[0]->fVel;
...
void MouseCursor::SetCursorPos(Point pPos)
{
    pCursorPos = pPos;
}
...
DragonScoreController::DragonScoreController
	(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c, bool bScoreShow)
	:BasicController(pGl_, rBound, c), t(5*nFramesInSecond), bClickToExit(false)
{
	if(bScoreShow)
	{
...
void DragonScoreController::DoubleClick()
{
	pGl->Next();
}
...
/*virtual*/ void AdvancedController::Update()
{
	...
	if(bPaused)
	{
		...
		Rectangle r(Point(rBound.sz.x/2 - (s1.size() * nCharWidth)/2 - 10, rBound.sz.y/2 - 10),
...
/*virtual*/ void Slime::Update()
{
    ...
    for(std::list< ASSP<ConsumableEntity> >::iterator itr = pAc->lsPpl.begin(); itr != pAc->lsPpl.end(); ++itr)
    {
        if(!(*itr)->bExist)
            continue;
...
/*virtual*/ void Slime::OnHit(char cWhat)
{
	if(pAc->nSlimeNum >= nSlimeMax && cWhat != 'M')
	{
        ...
        for(std::list< ASSP<Slime> >::iterator itr = pAc->lsSlimes.begin(); itr != pAc->lsSlimes.end(); ++itr)
		{
			if(!(*itr)->bExist)
				continue;
        ...
        for(std::list< ASSP<Sliminess> >::iterator itr = pAc->lsSliminess.begin(); itr != pAc->lsSliminess.end(); ++itr)
		{
			if(!(*itr)->bExist)
				continue;
        ...
        for(unsigned i = 0; i < vDeadSlimes.size(); ++i)
        {
            SP<FloatingSlime> pSlm = 
                new FloatingSlime(pAc->pGl->pr("slime_cloud"), vDeadSlimes[i], fAvg.ToPnt(), nFramesInSecond*1);
            pAc->AddBoth(pSlm);
        }
    ...
    bool bRevive = (cWhat != 'M');//pAc->nSlimeNum < nSlimeMax;// && nGeneration != 0;
	...
	SP<AnimationOnce> pAn = new AnimationOnce(dPriority,
		pAc->pGl->pr(bRevive ? "slime_die" : "slime_poke"),
        nFramesInSecond/5, GetPosition(), true);
    pAc->AddBoth(pAn);
	...
	for(int i = 0; i < 2; ++i)
	{
		...
		SP<Sliminess> pSlm = new Sliminess(GetPosition() + f.ToPnt(), pAc, false, nGeneration + 1);
		pAc->AddE(pSlm);
...
void ReadLevels(std::string sFile, Rectangle rBound, LevelStorage& vLvl)
{
    ...
    if(vLvl.size() == 0)
        throw SimpleException("<global>", "ReadLevels", "Cannot read levels at " + sFile);
...
TowerDataWrap::TowerDataWrap(ProgramEngine pe)
{
    ...
    
    {
        ...
        if(ifs.fail())
            throw SimpleException("TowerDataWrap", "<constructor>", "Need config.txt file!");
        ifs >> fp;
    }
    ...
    pCnt = new TwrGlobalController(pDr, pNum, pBigNum, pFancyNum, pSm, vLvl, rBound, this, fp);
    pCnt->StartUp();
} 
...
/*virutal*/ void SkellyGenerator::Update()
{
    if(t.Tick())
    {
        ...
        SP<Knight> pCr = new Knight(Critter(7, p, v, pAdv->rBound, 3, pAdv->pGl->pr("skelly"), true), pAdv, 'S');
        pAdv->AddBoth(pCr);
	...
	if(bAngry)
	{
		...
		else
		{
			...
			if(tSpell.Tick())
			{
				bCasting = false;
				Critter::fVel = fMvVel;
				Critter::seq = fMvVel.x < 0 ? pAc->pGl->pr("mage_f") :  pAc->pGl->pr("mage");
			}
		}
	}
...
void SlimeUpdater::Draw(SP<ScalingDrawer> pDr)
{
	pBuy->DrawSlimes();
}
...
MenuDisplay::MenuDisplay(Point pLeftTop_, SP<NumberDrawer> pNum_, SP<Animation> pMenuCaret_, SP<MenuController> pMenuController_, bool bCheatsUnlocked_)
    :pLeftTop(pLeftTop_), pNum(this, pNum_), pMenuCaret(this, pMenuCaret_),
    pMenuController(this, pMenuController_), pCurr(&memMain), bCheatsUnlocked(bCheatsUnlocked_)
{
	Size szSpacing(50, 10);
	memMain.vEntries.push_back(MenuEntry(szSpacing, "continue", &MenuDisplay::Continue));
	memMain.vEntries.push_back(MenuEntry(szSpacing, "restart", &MenuDisplay::Restart));
	memMain.vEntries.push_back(MenuEntry(szSpacing, "load chapter", &MenuDisplay::LoadChapterSubmenu));
	memMain.vEntries.push_back(MenuEntry(szSpacing, "options", &MenuDisplay::OptionsSubmenu));
	memMain.vEntries.push_back(MenuEntry(szSpacing, "exit", &MenuDisplay::Exit));

	memLoadChapter.vEntries.push_back(MenuEntry(szSpacing, "chapter 1", &MenuDisplay::Chapter1));
	memLoadChapter.vEntries.push_back(MenuEntry(szSpacing, "chapter 2", &MenuDisplay::Chapter2));
	memLoadChapter.vEntries.push_back(MenuEntry(szSpacing, "chapter 3", &MenuDisplay::Chapter3));
	memLoadChapter.vEntries.push_back(MenuEntry(szSpacing, "back", &MenuDisplay::Escape));

	memOptions.vEntries.push_back(MenuEntry(szSpacing, "",	&MenuDisplay::MusicToggle));
	nMusic = memOptions.vEntries.size() - 1;
    
	memOptions.vEntries.push_back(MenuEntry(szSpacing, "", &MenuDisplay::SoundToggle));
	nSound = memOptions.vEntries.size() - 1;

	memOptions.vEntries.push_back(MenuEntry(szSpacing, "",	&MenuDisplay::TutorialToggle));
	nTutorial = memOptions.vEntries.size() - 1;

	memOptions.vEntries.push_back(MenuEntry(szSpacing, "",	&MenuDisplay::FullScreenToggle));
	nFullScreen = memOptions.vEntries.size() - 1;

	if(bCheatsUnlocked)
	{
		memOptions.vEntries.push_back(MenuEntry(szSpacing, "",	&MenuDisplay::CheatsToggle));
		nCheats = memOptions.vEntries.size() - 1;
	}
	else
		nCheats = -1;

	memOptions.vEntries.push_back(MenuEntry(szSpacing, "back", &MenuDisplay::Escape));
	...
	//vOptionText.at(nFullScreen) = "full screen mode\n(relaunch for changes to take place)";
	if(bCheatsUnlocked)
		vOptionText.at(nCheats) = "f1-f10 to get bonuses\n\\ to skip level\ni for invincibility";
...
void MenuDisplay::PositionIncrement(bool bUp)
{
    ...
    while(true)
	{
		...
		if(pCurr->nMenuPosition == 0 || !pCurr->vEntries[pCurr->nMenuPosition].bDisabled)
			break;
	}

	if(pMenuController->pOptionText != 0)
		pMenuController->pOptionText->SetText(vOptionText.at(memOptions.nMenuPosition));
}

void MenuDisplay::Boop()
{
    if(pCurr->vEntries.at(pCurr->nMenuPosition).pTriggerEvent == &MenuDisplay::Escape)
...
void MenuDisplay::Restart()
{
    pMenuController->pGl->Restart();
}
...
void MenuDisplay::Exit()
{
	Trigger(pMenuController->pGl->pWrp->pExitProgram);
}
...
void MenuDisplay::LoadChapterSubmenu()
{
	pCurr = &memLoadChapter;
}

void MenuDisplay::OptionsSubmenu()
{
	pCurr = &memOptions;
}
...
{
    memOptions.vEntries.at(nMusic).sText = MusicString() + OnOffString(pMenuController->pGl->sbMusicOn.Get());
    memOptions.vEntries.at(nSound).sText = SoundString() + OnOffString(pMenuController->pGl->sbSoundOn.Get());
    memOptions.vEntries.at(nTutorial).sText = TutorialString() + OnOffString(pMenuController->pGl->sbTutorialOn.Get());
	...
	memOptions.vEntries.at(nFullScreen).sText = FullTextString() + OnOffString(bFullScreenSetting);// + sExtra;
	...
	memLoadChapter.vEntries.at(1).bDisabled = (pMenuController->pGl->snProgress.Get() < 1);
	memLoadChapter.vEntries.at(2).bDisabled = (pMenuController->pGl->snProgress.Get() < 2);
