...
namespace Gui
{
    ...
    struct IndexImgHelper: virtual public SP_Info
    {
    ...
    public:
        ...
        IndexImg():nI(-1), pCounter(0), pGr(0){}
        
        IndexImg(unsigned nI_, SP< IndexImgHelper > pGr_)
            :nI(nI_), pCounter(new unsigned(1)), pGr(pGr_.GetRawPointer()){}

        IndexImg(const IndexImg& i):nI(i.nI), pCounter(0), pGr(i.pGr)
        {
            if(i.pCounter)
                pCounter = &++*i.pCounter;
        }
    ...
    {
        SSP<GraphicalInterface<ImageHndl> > pGr;
    ...
    public:
        ...
        SimpleGraphicalInterface(SP<GraphicalInterface<ImageHndl> > pGr_):pGr(this, pGr_){}
...
//--------------------------------------------------------------------------------------------------
    ...
    void SimpleGraphicalInterface<ImageHndl>::DrawImage
        (Point p, IndexImg pImg, bool bRefresh)
    {pGr->DrawImage(p, Get(pImg), bRefresh);}

    template<class ImageHndl>
    void SimpleGraphicalInterface<ImageHndl>::DrawImage
        (Point p, IndexImg pImg, Rectangle r, bool bRefresh)
