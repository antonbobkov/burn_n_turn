...
struct Animation: public SimpleVisualEntity
{
    ...
    Animation(double dPriority_, const ImageSequence& seq, unsigned nTimeMeasure_, Point p)
        :SimpleVisualEntity(dPriority_, seq, false, nTimeMeasure_), pos(p){}
...
{
    Castle(Point p, Rectangle rBound_, ScalingDrawer* pDr)
        :Critter(10, p, Point(), rBound_, 0, ImageSequence(pDr, "castle.txt", "castle\\"))
    {}
};
...
struct AdvancedController: public BasicController
{
    ...
    AdvancedController(GlobalController* pGl_, Rectangle rBound, Color c);
...
struct CritterGenerator: virtual public EventEntity
{
    ...
    /*virtual*/ void Update()
    {
        if(double(rand())/RAND_MAX < dRate)
        {
            ...
            fPoint v = Point(rBound.sz.x / 2, rBound.sz.y / 2) - p;
            v.Normalize();
            p += rBound.p;
...
struct Dragon: public Critter
{
    ...
    Dragon(AdvancedController* pAd_):pAd(pAd_),
        imgStable(pAd_->pGl->pDr, "stable.txt", "dragon\\"),
        imgFly(pAd_->pGl->pDr, "fly.txt", "dragon\\"),
        Critter(10, Center(pAd_->rBound.sz), Point(), pAd_->rBound, 6, imgStable, 5U),
        bFly(false), bCarry(false)
    {
