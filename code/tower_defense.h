...
const float fGhostSpeedMultiplier = 1.3f; //where ghost speed = knightspeed*ghostspeedmultiplier
...
struct ScalingDrawer: public Drawer
{
    ...
    void Scale(Index& pImg, int nFactor_ = -1)
    {
        ...
        Index pRet = pGr->GetBlankImage(Size(pOrig->GetSize().x * nFactor_, pOrig->GetSize().y * nFactor_));
...
struct GameController: virtual public SP_Info
{
    ...
    GameController(SP<TwrGlobalController> pGl_, Rectangle rBound_ = Rectangle()):pGl(this, pGl_), rBound(rBound_){}
...
struct NumberDrawer: virtual public SP_Info
{
    ...
    NumberDrawer(SP<ScalingDrawer> pDr_, std::string sFontPath, std::string sFontName)
	...
	void DrawNumber(unsigned n, Point p, unsigned nDigits = 0)
    {
        ...
        {
            pDr->Draw(vImg[ vImgIndx['0' + vDigits[vDigits.size() - i - 1]] ], Point(p.x + 4 * i, p.y), false);
        }
    }
	*/
...
enum {BG_BACKGROUND = 0, BG_SLOW_BACKGROUND = 1, BG_BACKGROUND2 = 2, BG_SLOW_BACKGROUND2 = 3, BG_BACKGROUND3 = 4, BG_SLOW_BACKGROUND3 = 5};
...
template<class T>
class SavableVariable
{
	T var;
	std::string sFileName;
public:
	SavableVariable(std::string sFileName_, T var_default, bool bLoad = true):sFileName(sFileName_)
	{
		if(!bLoad)
			var = var_default;
		else
...
public:
	...
	SoundInterfaceProxy(SP<Soundic> pSndRaw_):pSndRaw(this, pSndRaw_), bSoundOn(true){}
...
struct TwrGlobalController: virtual public SP_Info
{
    ...
    TwrGlobalController(SP<ScalingDrawer> pDr_, SP<NumberDrawer> pNum_, SP<NumberDrawer> pBigNum_, SP<FontWriter> pFancyNum_,
		SP<Soundic> pSndRaw_, const LevelStorage& vLvl_, Rectangle rBound_, TowerDataWrap* pWrp_, FilePath fp = FilePath());
...
struct SimpleController: public GameController
{
    ...
    SimpleController(SP<TwrGlobalController> pGraph, std::string strFileName):GameController(pGraph)
    {
...
struct FlashingController: public GameController
{
    ...
    FlashingController(SP<TwrGlobalController> pGraph, std::string strFileName, std::string strTextName)
...
struct TextDrawEntity : virtual public VisualEntity
{
	...
	TextDrawEntity(float dPriority_, Point pos_, bool bCenter_, std::string sText, SP<NumberDrawer> pNum_)
...
struct TutorialTextEntity : virtual public EventEntity, public VisualEntity
{
	...
	TutorialTextEntity(float dPriority_, Point pos_, SP<NumberDrawer> pNum_, const bool* pIsTutorialOn_)
		:dPriority(dPriority_), pos(pos_), pNum(this, pNum_), t(1), nOffset(0), nTextVerticalSpacing(7), nDelta(1), pIsTutorialOn(pIsTutorialOn_)
	{
	}
	...
	#endif
#else
	const std::string sSteerMessage = "click and hold to steer"; // steer with joystick
	const std::string sShootingMessage = "click anywhere to shoot a fireball";  // move joystick to shoot a fireball
	const std::string sTakeOffMessage = "press space or click the tower to take off";  // press button to take off
#endif
...
struct TutorialLevelOne
{
	...
	TutorialLevelOne()
		:bKilledKnight(false), bFlying(false),
        bPrincessGenerated(false), bPrincessCaptured(false),
        pTexter(0){}
...
struct TutorialLevelTwo
{
	...
	TutorialLevelTwo()
		:bTraderGenerated(false), bTraderKilled(false),
        bBonusPickedUp(false),
        pTexter(0){}
...
struct SimpleVisualEntity : virtual public EventEntity, public VisualEntity
{
    ...
    SimpleVisualEntity(float dPriority_, const ImageSequence& seq_, bool bCenter_, unsigned nPeriod_) 
        : dPriority(dPriority_), seq(seq_), nPeriod(nPeriod_), t(nPeriod_ * seq_.GetTime()), bCenter(bCenter_),
        bTimer(true), bStep(false), bImageToggle(false)
    {}
    ...
    SimpleVisualEntity(float dPriority_, const ImageSequence& seq_, bool bCenter_, bool bStep_ = false) 
        ...
        bTimer(false), bStep(bStep_), nPeriod(1), bImageToggle(false)
    {}
...
struct SimpleSoundEntity : virtual public EventEntity
{
    ...
    SimpleSoundEntity(const SoundSequence& seq_, unsigned nPeriod_, SP<SoundInterfaceProxy> pSnd_) 
        :seq(seq_), nPeriod(nPeriod_), t(nPeriod * seq_.GetTime()), pSnd(this, pSnd_)
    {
    }
...
struct StaticImage: public VisualEntity
{
    ...
    StaticImage(Index img_, Point p_ = Point(0,0), bool bCentered_ = false, float dPriority_ = 0)
...
struct MenuEntry: virtual public SP_Info
{
	...
	MenuEntry(Size szSize_, std::string sText_, EvntPntr pTriggerEvent_, bool bDisabled_ = false)
		:szSize(szSize_), sText(sText_), pTriggerEvent(pTriggerEvent_), bDisabled(bDisabled_){}
};
...
struct MenuDisplay: virtual public EventEntity, public VisualEntity
{
	...
	MenuDisplay(Point pLeftTop_, SP<NumberDrawer> pNum_, SP<Animation> pMenuCaret_, SP<MenuController> pMenuController_, bool bCheatsUnlocked_);
	...
	/*virtual*/ void Draw(SP<ScalingDrawer> pDr)
    {
		...
		for(unsigned i = 0; i < pCurr->vEntries.size(); ++i)
		{
			if(!pCurr->vEntries[i].bDisabled)
				...
				pNum->DrawColorWord(pCurr->vEntries[i].sText, p, Color(125,125,125), false);
	...
	/*virtual*/ void Update()
	{
		pCurr->vEntries.at(0).bDisabled = false;
	}
...
struct Critter: virtual public PhysicalEntity, public SimpleVisualEntity
{
    ...
    Critter(unsigned nRadius_, fPoint fPos_, fPoint fVel_, Rectangle rBound_,
        ...
        nRadius(nRadius_), fPos(fPos_), fVel(fVel_), bDieOnExit(true), sUnderText(""){} 
...
struct FancyCritter: virtual public PhysicalEntity, public SimpleVisualEntity
{
    ...
    FancyCritter(unsigned nRadius_, fPoint fPos_, fPoint fVel_, Rectangle rBound_,
        ...
        nRadius(nRadius_), fPos(fPos_), fVel(fVel_), bDieOnExit(true), tm(nPeriod){} 
};
...
struct MouseCursor
{
    ...
    MouseCursor(ImageSequence imgCursor_, Point pCursorPos_, TwrGlobalController* pGl_)
        :imgCursor(imgCursor_), pCursorPos(pCursorPos_), pGl(pGl_), bPressed(false){}
...
struct MenuController: public BasicController
{
	...
	MenuController(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c, int nResumePosition_)
		:BasicController(pGl_,rBound,c),nResumePosition(nResumePosition_), pMenuDisplay(this, 0),
        mc(pGl->pr("claw"), Point(), pGl.GetRawPointer()),
...
{
	StartScreenController(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c):BasicController(pGl_,rBound,c){}
...
struct BuyNowController: public BasicController
{
	...
	BuyNowController(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c):BasicController(pGl_,rBound,c),t(120),nSlimeCount(50),
		tVel(nFramesInSecond/2)
    {
		for(int i = 0; i < nSlimeCount; i++)
		{
			mSlimes.push_back(new Animation(0, pGl->pr("slime"), nFramesInSecond/10, Point(rBound.sz.x/2,rBound.sz.y/2+25), true));
...
struct Cutscene: public BasicController
{
    ...
    Cutscene(SP<TwrGlobalController> pGl_, Rectangle rBound_, std::string sRun, std::string sChase, bool bFlip = false)
        :BasicController(pGl_, rBound_, Color(0,0,0)), pCrRun(this, 0), pCrFollow(this, 0)
		, bRelease(false), tm(nFramesInSecond/5), Beepy(true)
    {
		...
		SP<FancyCritter> pCr1 = new FancyCritter(7, fPoint(xPos,rBound_.sz.y/2), fPoint(m*10, 0), rBound, 3, seq1, nFramesInSecond/5);
        AddBoth(pCr1);
        ...
        SP<FancyCritter> pCr2 = new FancyCritter(7, fPoint(xPos,rBound_.sz.y/2), fPoint(m*12, 0), rBound, 3, seq2, nFramesInSecond/5);
    ...
    /*virtual*/ void Update()
    {
        if(!bRelease && pCrRun->GetPosition().x >= rBound.sz.x/3 && pCrRun->GetPosition().x <= rBound.sz.x*2/3)
        {
            bRelease = true;
...
struct FireballBonus: virtual public Entity
{
    ...
    template<class T>
    FireballBonus(int nNum_, std::string str, T t)
		:nNum(nNum_){Add(str, t);}
...
struct BonusScore: public EventEntity, public VisualEntity
{
    ...
    BonusScore(SP<AdvancedController> pAc_, Point p_, unsigned nScore_)
		:p(p_ + Point(0, -5)), t(unsigned(.1F * nFramesInSecond)), nC(0), pAc(this, pAc_), c(255, 255, 0), nScore(nScore_), nScoreSoFar(0)
    {
...
struct PositionTracker
{
	...
	void On(){bPressed = true; nCounter = 0;}
	int Off(){bPressed = false; return nCounter;}
	void Update(){if(bPressed) ++nCounter;}
	
	void UpdateMouse(Point pMouse_){pMouse = pMouse_; pMouse.x /= 2; pMouse.y /= 2;}
	void UpdateLastDownPosition(Point pMouse_){pLastDownPosition = pMouse_; pLastDownPosition.x /= 2; pLastDownPosition.y /= 2;}
	fPoint GetDirection(fPoint fDragonPos){return pMouse - fDragonPos;}
	fPoint GetFlightDirection(fPoint fDragonPos){return pLastDownPosition - fDragonPos;}
};
...
struct AdvancedController: public BasicController
{
    ...
    AdvancedController(const AdvancedController& a)
		:BasicController(a), tr(a.tr), t(a.t), bCh(a.bCh), nLvl(a.nLvl), nSlimeNum(a.nSlimeNum),
		bPaused(a.bPaused), tLoseTimer(a.tLoseTimer), bGhostTime(a.bGhostTime), 
		tBlink(a.tBlink), bBlink(a.bBlink), pGr(a.pGr), tStep(a.tStep), bLeft(a.bLeft), pSc(a.pSc),bLeftDown(a.bLeftDown),bRightDown(a.bRightDown),nLastDir(a.nLastDir),bWasDirectionalInput(a.bWasDirectionalInput),
		pt(a.pt), mc(a.mc), bFirstUpdate(true), pTutorialText(this, a.pTutorialText), tutOne(a.tutOne), tutTwo(a.tutTwo), bTimerFlash(a.bTimerFlash),
		pMgGen(a.pMgGen)
    {
    ...
    AdvancedController(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c, const LevelLayout& lvl);
...
struct DragonScoreController: public BasicController
{
	...
	DragonScoreController(SP<TwrGlobalController> pGl_, Rectangle rBound, Color c, bool bScoreShow);
...
struct Fireball: public Critter
{
        ...
        pBc(this, pBc_), fb(fb_), ch(ch_), nChain(nChain_)
    {
        ...
        else
        {
            ...
            Critter::seq = ImageSequence(pBc->pGl->pr("laser" + GetSizeSuffix(fb)).vImage[n]);
        }
    }
            ...
            if (this->HitDetection(*itr))
            {
                ...
                else
                {
                    ...
                    SP<KnightOnFire> pKn = new KnightOnFire(
                        ...
                        rBound, 1.F, ImageSequence(), true), pBc, 15 * nFramesInSecond, Chain(fb.uMap["setonfire"]));
                    pBc->AddBoth(pKn);
                }
                ...
                if(!bMultiHit)
				{
					...
					if(bKeepGoing)
					{
						...
						{
							SP<Fireball> pFb = new Fireball((*itr)->GetPosition(),
								GetWedgeAngle(v, 1.F/6, i, nChain), pBc, fb, Chain(), nChain);
							pBc->AddBoth(pFb);
						}
					}
					...
					if(fb.uMap["explode"] > 0)
					{
						...
						if(!fb.bMap["laser"])
						{
							pEx = new ChainExplosion(
								...
								GetExplosionInitialRaduis(fb),
								GetExplosionExpansionRate(fb),
...
struct CircularFireball: virtual public Fireball, virtual public TimedFireballBonus
{
    float fRadius;
    fPoint i_pos;
    Timer t;
	...
	CircularFireball(const Fireball& f, float fRadius_, unsigned nPeriod)
		:Fireball(f),
		 TimedFireballBonus(FireballBonus(8, false), nPeriod),
...
struct SkellyGenerator: public EventEntity
{
    ...
    SkellyGenerator(Point p_, SP<AdvancedController> pAdv_):p(p_), t(unsigned(.7F * nFramesInSecond)), pAdv(this, pAdv_)
    {
		SP<AnimationOnce> pSlm = new AnimationOnce(2.F, pAdv->pGl->pr("skelly_summon"), unsigned(.1F * nFramesInSecond), p_, true);
        pAdv_->AddBoth(pSlm);
    }
...
struct Mage: public Critter, public ConsumableEntity
{
    ...
    Mage(const Critter& cr, SP<AdvancedController> pAc_, bool bAngry_)
		...
		tUntilSpell(GetTimeUntillSpell()), tSpell(3*nFramesInSecond), tSpellAnimate(unsigned(.7F*nFramesInSecond))
	{
		fMvVel = Critter::fVel;
...
struct FireballBonusAnimation: public Animation, virtual public PhysicalEntity
{
    ...
    FireballBonusAnimation(Point p_, unsigned n_, SP<AdvancedController> pAd_)
		:Animation(.5F, ImageSequence(), nFramesInSecond/10, p_, true), n(n_), bBlink(false), pAd(this, pAd_),
		tm(nBonusOnGroundTime), sUnderText("")
    {
...
struct Trader:  public Critter, public ConsumableEntity
{
    ...
    Trader(const Critter& cr, SP<AdvancedController> pAc_, bool& bFirstBns_):Critter(cr), pAc(this, pAc_), bFirstBns(bFirstBns_)
	{
	}
    ...
    /*virtual*/ void OnHit(char cWhat)
    {
        ...
        SP<FireballBonusAnimation> pFb = new FireballBonusAnimation(GetPosition(), RandomBonus(false), pAc);
		if(bFirstBns)
		{
			pFb->sUnderText = "loot";
			bFirstBns = false;
...
struct Knight: public Critter, public ConsumableEntity
{
    ...
    Knight(const Critter& cr, SP<AdvancedController> pAc_, char cType_)
		:Critter(cr), pAc(this, pAc_), cType(cType_), nGhostHit(1), nGolemHealth(nGolemHealthMax)
	{
	}
    ...
    /*virtual*/ void Update()
    {
		...
		if(cType == 'S')
		{
			...
			for(std::list< ASSP<FireballBonusAnimation> >::iterator itr = pAc->lsBonus.begin(); itr != pAc->lsBonus.end(); ++itr)
			{
				if(!(*itr)->bExist)
					continue;
...
struct MegaSlime: public Critter, public ConsumableEntity
{
	...
	MegaSlime(fPoint fPos, Rectangle rBound, SP<AdvancedController> pAc_)
		:Critter(8, fPos, fPoint(0,0), rBound, 3, pAc_->pGl->pr("megaslime"), nFramesInSecond/5),
...
struct Ghostiness: public EventEntity
{
    ...
    Ghostiness(Point p_, SP<AdvancedController> pAdv_, Critter knCp_, int nGhostHit_)
...
struct Slime: public Critter, public ConsumableEntity
{
    ...
    Slime(fPoint fPos, Rectangle rBound, SP<AdvancedController> pAc_, int nGeneration_)
...
struct Sliminess: public EventEntity
{
    ...
    Sliminess(Point p_, SP<AdvancedController> pAdv_, bool bFast_, int nGeneration_)
		:p(p_), pAdv(this, pAdv_), bFast(bFast_),nGeneration(nGeneration_), pSlm(this, 0)
    {
		ImageSequence seq = bFast ? pAdv->pGl->pr("slime_reproduce_fast") : pAdv->pGl->pr("slime_reproduce");

		t = bFast ? Timer(unsigned(1.3F  * nFramesInSecond)) : Timer(unsigned(2.3F * nFramesInSecond));
		
		SP<AnimationOnce> pSlmTmp = new AnimationOnce(2.F, seq, unsigned(.1F * nFramesInSecond), p_, true);
		pSlm = pSlmTmp;
...
public:
    SegmentSimpleException(crefString strFnName_, crefString strProblem_)
        :MyException("SegmentSimpleException", "BrokenLine", strFnName_), strProblem(strProblem_){}
...
struct BrokenLine
{
    ...
    {Add(r.p); Add(fPoint(float(r.Left()), float(r.Bottom()))); Add(fPoint(float(r.Right()), float(r.Top()))); Add(r.GetBottomRight()); CloseLast();}

    fPoint RandomByLength()
    {
        if(vEdges.empty())
            throw SegmentSimpleException("RandomByLength", "Invalid (empty) segment arrays");
    ...
    {
        if(vEdges.empty())
            throw SegmentSimpleException("RandomBySegment", "Invalid (empty) segment arrays");
...
struct KnightGenerator: virtual public EventEntity
{
    ...
    KnightGenerator(float dRate_, Rectangle rBound_, SP<AdvancedController> pBc_, const BrokenLine& bl_)
        :dRate(dRate_), rBound(rBound_), pBc(this, pBc_), seq(pBc_->pGl->pr("knight")), bl(bl_), tm(1), bFirst(false)
	{
    ...
    void Generate(bool bGolem = false)
	{
		...
		if(bGolem)
		{
			pCr = new Knight(Critter(14, p, v*.5, rBound, 3,
				v.x < 0 ? pBc->pGl->pr("golem") : pBc->pGl->pr("golem_f"),
				true), pBc, 'W');
		}
		else if(pBc->bGhostTime)// && (pBc->nLvl <= 6)) we want BOTH ghosts and golems
		{
...
struct PrincessGenerator: virtual public EventEntity
{
    ...
    PrincessGenerator(float dRate_, Rectangle rBound_, SP<AdvancedController> pBc_)
        :dRate(dRate_), rBound(rBound_), pBc(this, pBc_), tm(GetRandTimeFromRate(dRate_)),bFirst(false)
	{
		if(pBc->nLvl == 1 && pBc->pGl->nHighScore == 0)
			bFirst = true;
...
struct MageGenerator: virtual public EventEntity
{
    ...
    MageGenerator(float dRate_, float dAngryRate_, Rectangle rBound_, SP<AdvancedController> pBc_)
...
struct TraderGenerator: virtual public EventEntity
{
    ...
    TraderGenerator(float dRate_, Rectangle rBound_, SP<AdvancedController> pBc_)
        :dRate(dRate_), rBound(rBound_), pBc(this, pBc_), tm(GetRandTimeFromRate(dRate_)), bFirst(false), bFirstBns(false)
	{
		if(pBc->nLvl == 1 && pBc->pGl->nHighScore == 0)
		{
			bFirst = true;
    ...
    /*virtual*/ void Update()
    {
        if(tm.Tick())
        {
            ...
            SP<Trader> pCr = new Trader(Critter(7, p, vel, rBound, 3,
...
inline unsigned RandomBonus(bool bInTower /* = true*/)
{
	...
	if(bInTower) v.push_back(1.F); // ring fireball
...
struct Dragon: public Critter
{
    ...
    SP<TimedFireballBonus> GetBonus(unsigned n, unsigned nTime)
    {
        ...
        if(n == 0)
           pBonus = new TimedFireballBonus(FireballBonus(n, "regenerate", 2U), nTime*2);
		else if(n == 1)
		...
		{
            pBonus = new TimedFireballBonus(FireballBonus(n, "total", nFireballsPerBonus), nTime*2);
			//++nExtraFireballs;
		}
        else if(n == 5)
		...
		*/
        ...
        else if(n == 8)
        {
			...
			//fb.fMap["speed"] = (4.F + fb.uMap["regenerate"]*2.F) * 10 / nFramesInSecond;
			...
			{
				SP<CircularFireball> pFb = new CircularFireball(Fireball(p, GetWedgeAngle(fVel, 1.F, i, nNumCirc + 1), pAd, fb,
					//Chain(true /*fb.uMap["fireballchain"]*/), fb.uMap["fireballchainnum"]), 40, nTime);
					Chain(), GetFireballChainNum(fb)), 35, nTime*2);
				pAd->AddBoth(pFb);
			}
        ...
        else if(n == 9)
        {
			...
			for(std::list< ASSP<ConsumableEntity> >::iterator itr = pAd->lsPpl.begin(),
                ...
                if((*itr)->GetType() == 'K' || (*itr)->GetType() == 'S' || (*itr)->GetType() == 'L')
				{
					if((*itr)->GetType() == 'K' && GetAllBonuses().uMap["setonfire"] != 0)
					{
						(*itr)->bExist = false;
						SP<KnightOnFire> pKn = new KnightOnFire(
							Critter((*itr)->GetRadius(), (*itr)->GetPosition(), fPoint(),
							rBound, 1.F, ImageSequence(), true), pAd, 15 * nFramesInSecond, Chain(GetAllBonuses().uMap["setonfire"]));
						pAd->AddBoth(pKn);
					}
					else
						(*itr)->OnHit('F');
	...
	void FlushBonuses()
	{
        for(std::list< ASSP<TimedFireballBonus> >::iterator itr = lsBonuses.begin(), etr = lsBonuses.end();
            itr != etr; ++itr)
    ...
    Dragon(SP<Castle> pCs_, SP<AdvancedController> pAd_,
		...
		nFireballCount(0), tFireballRegen(1), bTookOff(false), nPrCr(0), nExtraFireballs(0),
		...
		else
		{
			...
			if(int(tFireballRegen.nTimer) >= nPeriod)
			{
				...
				if(nFireballCount < int(fb.uMap["total"]))
				{
					...
					if(nInitialRegen == 0) //this is so 0 fireball regen will not be limited by framerate
    ...
    void Fire(fPoint fDir)
    {
        ...
        for(unsigned i = 0; i < nNumber; ++i)
        {
			...
			if(nNumber % 2 == 0 && (i == nNumber/2 || i == nNumber/2-1))
			{
				...
				if(i == nNumber/2)
					pPos = Point(Crd(pPos.x + fNormalized.y*6), Crd(pPos.y - fNormalized.x*6));
				else
					pPos = Point(Crd(pPos.x - fNormalized.y*6), Crd(pPos.y + fNormalized.x*6));
			}
			
			SP<Fireball> pFb =
				new Fireball(pPos, fShoot, pAd, fb,
    ...
    void Toggle()
    {
        if(!bFly)
        {
			...
			fVel = pAd->pt.GetFlightDirection(GetPosition());//GetDirection(GetPosition());
