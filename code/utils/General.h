 ...
 * Provides: math min/max/abs; exception hierarchy (MyException,
 * SimpleException); parsing helpers; Timer; stream/file handlers and FilePath;
 * Record/RecordKeeper persistence; Index and IndexKeeper; GuiKeyType enum.
 */
...
struct Timer {
  ...
  unsigned UntilTick() { return nPeriod - nTimer % nPeriod; }
  ...
  void NextTick() { nTimer = nPeriod - 1; }
};
