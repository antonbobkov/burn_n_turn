...
public:
  ...
  template <class N> SP(const SP<N> &pSmPnt) { CONSTRUCT(pSmPnt.pPointTo); }

  template <class N> SP(const SSP<N> &pPrmPnt);

  SP<T> &operator=(T *pPointTo_) { return EQUAL(pPointTo_); }
  SP<T> &operator=(const SP<T> &pSmPnt) { return EQUAL(pSmPnt.pPointTo); }

  template <class N> SP<T> &operator=(const SP<N> &pSmPnt) {
    return EQUAL(pSmPnt.pPointTo);
  }

  template <class N> SP &operator=(const SSP<N> &pPrmPnt);
  ...
  bool operator==(const SP<T> &pSmPnt) { return pPointTo == pSmPnt.pPointTo; }
...
public:
  ...
  template <class N>
  SSP(SP_Info *pHost_, const SP<N> &pSmrtPnt) : SSP_Base(pHost_) {
  ...
  template <class N> SSP<T> &operator=(const SP<N> &pSmrtPnt) {
...
template <class T> template <class N> SP<T>::SP(const SSP<N> &pPrmPnt) {
  CONSTRUCT(pPrmPnt.pPointTo);
}

template <class T>
template <class N>
SP<T> &SP<T>::operator=(const SSP<N> &pPrmPnt) {
