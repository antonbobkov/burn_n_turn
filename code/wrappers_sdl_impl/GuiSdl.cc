...
SdlImage::SdlImage(SDL_Surface *pImg_)
    : Image((PointerAssert<ImageNullException>("SdlImage", "Constructor",
                                               "pImg_", pImg_),
             Size(pImg_->w, pImg_->h))),
      pImg(pImg_) {}

SdlImage::SdlImage(Size sz_) : Image(sz_) {
  // pImg = SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_SRCALPHA, sz_.x, sz_.y, 32,
  // 0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000);
  pImg = SDL_CreateRGBSurface(0, sz_.x, sz_.y, 32, 0x00ff0000, 0x0000ff00,
                              0x000000ff, 0xff000000);

  if (!pImg)
    throw SimpleSdlImageException("constructor",
                                  "Cannot create empty SDL surface" +
                                      SdlImageException::GetSdlError());
}
...
SdlGraphicalInterface::SdlGraphicalInterface(
    Size sz_, bool bFullScreen /* = false*/,
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0)
    throw SimpleSGIException("constructor",
                             "Cannot initialize SDL video library" +
                                 SimpleSGIException::GetSdlError());

  pScreenWindow = SDL_CreateWindow("Dragon Game", SDL_WINDOWPOS_UNDEFINED,
                                   SDL_WINDOWPOS_UNDEFINED, sz.x, sz.y,
                                   bFullScreen ? SDL_WINDOW_FULLSCREEN : 0);

  SDL_Surface *pScrImg = SDL_GetWindowSurface(pScreenWindow);
  /*
  if(bFullScreen)
          pScrImg = SDL_SetVideoMode(sz.x, sz.y, 32, SDL_DOUBLEBUF |
  SDL_HWSURFACE | SDL_FULLSCREEN); else pScrImg = SDL_SetVideoMode(sz.x, sz.y,
  32, SDL_DOUBLEBUF | SDL_HWSURFACE);
          */

  if (!pScrImg)
    throw SimpleSGIException("constructor",
                             "Cannot set SDL video mode for the screen" +
                                 SimpleSGIException::GetSdlError());
...
void SdlGraphicalInterface::DeleteImage(SdlImage *pImg) { delete pImg; }

Image *SdlGraphicalInterface::GetImage(SdlImage *pImg) const { return pImg; }
    ...
    try
    {
        ...
        for(Crd x = 0; x < sz.x; ++x)
                    for(Crd y = 0; y < sz.y; ++y)
                                pRetImg->SetPixel(Point(x,y),
pImg->GetPixel(Point(x,y)));
...
SdlImage *SdlGraphicalInterface::LoadImage(std::string sFileName) {
  ...
  if (!pImg)
    throw SimpleSGIException("LoadImage",
                             "Cannot load bmp image from file " + sFileName +
                                 SimpleSGIException::GetSdlError());

  SDL_Surface *pRet = SDL_ConvertSurface(pImg, &pxf, 0); // SDL_HWSURFACE);

  if (!pRet)
    throw SimpleSGIException(
        "LoadImage",
        "Cannot convert image to the usable format (Loaded from: " + sFileName +
            ")" + SimpleSGIException::GetSdlError());
...
void SdlGraphicalInterface::SaveImage(std::string sFileName, SdlImage *pImg) {
  PointerAssert<NullPointerSGIException>("SdlGraphicalInterface", "SaveImage",
                                         "pImg", pImg);
  ...
  if (!pSv)
    throw SimpleSGIException(
        "SaveImage",
        "Cannot convert image to the saveable format (Trying to save into: " +
            sFileName + ")" + SimpleSGIException::GetSdlError());

  if (SDL_SaveBMP(pSv, sFileName.c_str()) < 0)
    throw SimpleSGIException("SaveImage",
                             "Failed to save an image into " + sFileName +
                                 SimpleSGIException::GetSdlError());

  SDL_FreeSurface(pSv);
}

void SdlGraphicalInterface::DrawImage(Point p, SdlImage *pImg, Rectangle r,
                                      bool bRefresh /* = true*/) {
  PointerAssert<NullPointerSGIException>("SdlGraphicalInterface", "DrawImage",
                                         "pImg", pImg);
...
void SdlGraphicalInterface::DrawRectangle(Rectangle p, Color c,
                                          bool bRefresh /*= true*/) {
  p.p += rOffSet.p;
  ...
  try {
    ...
    SimpleSGIException exThr("DrawRectangle", "Cannot draw a reactangle " +
                                                  RectangleToString(p) +
...
void SdlGraphicalInterface::RectangleOnto(SdlImage *pImg, Rectangle p,
                                          Color c) {
  PointerAssert<NullPointerSGIException>("SdlGraphicalInterface",
                                         "RectangleOnto", "pImg", pImg);

  pImg->Unlock();
  p = Intersect(p, Rectangle(pImg->GetSize()));
  SDL_Rect r = ToSdl(p);
  if (0 > SDL_FillRect(pImg->pImg, &r,
                       SDL_MapRGB(pImg->pImg->format, c.R, c.G, c.B)))
    throw SimpleSGIException("RectangleOnto",
                             "Cannot draw a rectangle in an image" +
                                 SimpleSGIException::GetSdlError());
}
/*
...
void SdlGraphicalInterface::RefreshAll() {
  if (rOffSet.p != Point() || rOffSet.sz != sz) {
    RectangleOnto(pScreenImage, Rectangle(Point(0, 0), Size(rOffSet.p.x, sz.y)),
                  Color(0, 0, 0));
    RectangleOnto(pScreenImage, Rectangle(Point(0, 0), Size(sz.x, rOffSet.p.y)),
                  Color(0, 0, 0));

    RectangleOnto(pScreenImage,
                  Rectangle(Point(rOffSet.Right(), 0),
                            Size(sz.x - rOffSet.Right(), sz.y)),
                  Color(0, 0, 0));

    RectangleOnto(pScreenImage,
                  Rectangle(Point(0, rOffSet.Bottom()),
                            Size(sz.x, sz.y - rOffSet.Bottom())),
                  Color(0, 0, 0));
  }
...
void SdlGraphicalInterface::ImageOnto(SdlImage *pImgDest, Point p,
                                      SdlImage *pImgSrc, Rectangle r) {
  PointerAssert<NullPointerSGIException>("SdlGraphicalInterface", "ImageOnto",
                                         "pImgDest", pImgDest);
  PointerAssert<NullPointerSGIException>("SdlGraphicalInterface", "ImageOnto",
                                         "pImgSrc", pImgSrc);
