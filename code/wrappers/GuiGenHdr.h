...
struct Color {
  ...
  Color(Byte R_, Byte G_, Byte B_, Byte nTransparent_ = Byte(255))
      : R(R_), G(G_), B(B_), nTransparent(nTransparent_) {}
  Color(int R_, int G_, int B_, int nTransparent_ = 255) // for convenience
      : R(Byte(R_)), G(Byte(G_)), B(Byte(B_)),
        nTransparent(Byte(nTransparent_)) {}
};
...
inline std::ostream &operator<<(std::ostream &ofs, Point f) {
  return ofs << f.x << " " << f.y;
}
inline std::istream &operator>>(std::istream &ifs, Point &f) {
  return ifs >> f.x >> f.y;
}
...
inline fPoint &operator+=(fPoint &f1, const fPoint &f2) {
  f1.x += f2.x;
  f1.y += f2.y;
  return f1;
}
...
inline fPoint operator+(const fPoint &f1, const fPoint &f2) {
  fPoint f(f1);
  return f += f2;
}
inline fPoint operator-(const fPoint &f1, const fPoint &f2) {
...
inline bool operator==(const fPoint &f1, const fPoint &f2) {
  return (f1.x == f2.x) && (f1.y == f2.y);
}
inline bool operator!=(const fPoint &f1, const fPoint &f2) {
  return (f1.x != f2.x) || (f1.y != f2.y);
}
inline fPoint &operator/=(fPoint &f1, float f) {
  f1.x /= f;
  f1.y /= f;
  return f1;
}
...
inline fPoint operator/(const fPoint &f1, float f) {
  fPoint ff(f1);
  return ff /= f;
}
inline fPoint operator*(const fPoint &f1, float f) {
...
inline float Dot(const fPoint &f1, const fPoint &f2) {
  return f1.x * f2.x + f1.y * f2.y;
}

inline std::ostream &operator<<(std::ostream &ofs, fPoint f) {
  ofs << f.x << " " << f.y;
  return ofs;
}
inline std::istream &operator>>(std::istream &ifs, fPoint &f) {
  ifs >> f.x >> f.y;
  return ifs;
}
...
inline std::ostream &operator<<(std::ostream &ofs, Size s) {
  return ofs << s.x << " " << s.y;
}
inline std::istream &operator>>(std::istream &ifs, Size &s) {
  return ifs >> s.x >> s.y;
}
...
struct Rectangle {
  ...
  Rectangle(Point p1, Point p2) : p(p1), sz(Size(p2.x - p1.x, p2.y - p1.y)) {
    Normalize();
  }
  Rectangle(Crd l, Crd t, Crd r, Crd b) : p(l, t), sz(r - l, b - t) {
    Normalize();
  }
...
bool InsideRectangle(Rectangle r,
                     Point p); // is the point inside the rectangle?
