...
template<class T>
TerminatorEvent<T>* NewTerminatorEvent(T* pObject_){return new TerminatorEvent<T>(pObject_);}
...
template<class A, class B>
SwitchEvent<A,B>* NewSwitchEvent(A& objCurr_, B& objNew_){return new SwitchEvent<A,B>(objCurr_, objNew_);}

template<class A, class B>
class CpSwitchEvent : public Event
{
    A& objCurr;
    B objNew;
public:
...
template<class A, class B>
CpSwitchEvent<A,B>* NewCpSwitchEvent(A& objCurr_, B objNew_){return new CpSwitchEvent<A,B>(objCurr_, objNew_);}
...
struct SequenceOfEvents: public Event
{
    ...
    /*virtual*/ void Trigger();
    SequenceOfEvents(){}
    SequenceOfEvents(SP<Event> pEv1, SP<Event> pEv2){vEv.push_back(pEv1); vEv.push_back(pEv2);}
    SequenceOfEvents(SP<Event> pEv1, SP<Event> pEv2, SP<Event> pEv3)
		{vEv.push_back(pEv1); vEv.push_back(pEv2); vEv.push_back(pEv3);}
};
...
public:
    ...
    MakeSoundEvent(SP<SoundInterface<Index> > pSn_, Index Snd_): pSn(pSn_.GetRawPointer()), Snd(Snd_){}
    MakeSoundEvent(SoundInterface<Index>* pSn_, Index Snd_): pSn(pSn_), Snd(Snd_){}
