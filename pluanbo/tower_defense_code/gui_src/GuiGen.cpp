...
namespace Gui
{
    ...
    FontWriter::FontWriter(FilePath& fp, std::string sFont, SP<GraphicalInterface<Index> > pGr_, unsigned nZoom)
        :vImgIndx(256, -1), pGr(pGr_)
    {
        ...
        for(unsigned i = 0; i < n; ++i)
        {
            ...
            for(p.x = 0; p.x < szSymbol.x; ++p.x)
                pCurr->SetPixel(p, pImg->GetPixelSafe(Point(i * (szSymbol.x + nGap) + p.x, p.y)));
    ...
    void FontWriter::DrawColorWord(std::string s, Point p,
        Color c/* = Color(0,0,0,0)*/, bool bCenter/* = false*/, bool bRefresh/* = false*/)
    {
        if(bCenter)
        {
